...
void XArrayList<T>::copyFrom(const XArrayList<T> &list)
{
    ...
    for (int i = 0; i < this->count; i++) {
        if constexpr (std::is_pointer<T>::value) {  
            this->data[i] = new typename std::remove_pointer<T>::type(*(list.data[i]));  // Sao chép nội dung của đối tượng mà con trỏ trỏ tới
        } 
        else {
            this->data[i] = list.data[i];   // Nếu không phải con trỏ, sao chép bình thường
        }
    }
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    if (index<0)
        throw std::out_of_range("Index is out of range!");
    if (index>=capacity){
        int newCapacity = max(capacity * 2,index+1) ;
        try {
